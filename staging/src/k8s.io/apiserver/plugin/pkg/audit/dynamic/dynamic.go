...
import (
	...
	legacyenforcedplugin "k8s.io/apiserver/plugin/pkg/audit/enforced/legacy"
...
func NewEnforcedBackend(c *Config) (audit.EnforcedBackend, error) {
	...
	if c.StaticBackend != nil {
		enforced := legacyenforcedplugin.NewBackend(c.StaticBackend, c.StaticPolicyChecker)
		staticBackend = &Backend{nil, enforced, make(chan struct{})}
		backends[staticUID] = staticBackend
	}

	cm, err := webhook.NewClientManager(auditinternal.SchemeGroupVersion, auditinternal.AddToScheme)
	if err != nil {
		return nil, err
	}
