...
type Config struct {
	...
	AuthInfoResolverWrapper webhookconfig.AuthenticationInfoResolverWrapper
}
...
// NewBackend returns a backend that dynamically updates its configuration based on a
...
// ProcessEvents proccesses the given events per current backend map
func (d *dynamic) ProcessEvents(events ...*audit.Event) {
	for _, b := range d.GetBackends() {
		b.Backend.processEvents(events)
	}
}

// processEvents enforces policy on each event and sends to the delegated backend
func (d *dynamic) processEvents(events ...*audit.Event) {
	for _, event := range events {
		audit.ObservePolicyLevel(b.backend.Spec.Policy.Level)
		if level == auditinternal.LevelNone {
			continue
		}
		event.Event = policy.EnforcePolicy(event.Event, b.backend.Spec.Policy.Level, b.backend.Spec.Policy.OmitStages)
		if event.Event == nil {
			continue
		}
		b.Backend.ProcessEvents(event.Event)
	}
}
...
func (d *dynamic) Run(stopCh <-chan struct{}) error {
	...
	for _, b := range d.GetBackends() {
		funcs = append(funcs, func() error {
			return b.backend.Run(b.stopChan)
		})
	}
...
func (d *dynamic) createBackend(configuration *auditregv1alpha1.AuditSink) (*Backend, error) {
	...
	err = backend.backend.Run(backend.stopChan)
	if err != nil {
		return nil, err
	}
	return backend, nil
