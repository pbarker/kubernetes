...
const PluginName = "legacy_enforced"

// Backend filters audit events by legacy policy trimming them as necesssary
...
func (b Backend) ProcessEnforcedEvents(events ...*audit.EnforcedEvent) {
	for _, event := range events {
		level, stages := b.policyChecker.LevelAndStages(event.Attributes)
		audit.ObservePolicyLevel(level)
		if level == auditinternal.LevelNone {
			continue
		}
		event.Event = policy.EnforcePolicy(event.Event, level, stages)
		if event.Event == nil {
			continue
		}
