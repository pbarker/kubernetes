...
func TestDynamicAudit(t *testing.T) {
	...
	require.NoError(t, testServer1.Health(wait.ForeverTestTimeout), "server1 never became healthy")
	require.NoError(t, testServer2.Health(wait.ForeverTestTimeout), "server2 never became healthy")
	...
	// This test will run a background process the generates audit events sending them to a sink.
	// Whilst that generation is occuring, the sink is updated to point to a different server.
...
func simpleOp(name string, kubeclient kubernetes.Interface) ([]utils.AuditEvent, error) {
	_, _ = kubeclient.CoreV1().ConfigMaps(namespace).Get(name, metav1.GetOptions{})
